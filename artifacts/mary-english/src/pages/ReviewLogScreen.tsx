...
function EntryCard({ entry, index }: { entry: ReviewLogEntry; index: number }) {
  return (
    <motion.div
    ...
    >
      <div className="flex items-center justify-between px-4 pt-4 pb-2">
        <span className={`text-xs font-bold px-2.5 py-1 rounded-full ${TASK_TYPE_COLORS[entry.taskType]}`} data-testid={`entry-tasktype-${entry.id}`}>
          {entry.taskType}
        </span>
      ...
      <div className="px-4 pb-4 space-y-3">
        ...
        <div className="flex gap-3 items-start">
          <span className="text-xs font-bold text-accent-foreground shrink-0 mt-0.5 w-20 text-right leading-5">Mary:</span>
          <p className="text-sm text-foreground/80 leading-relaxed flex-1 italic" data-testid={`entry-mary-${entry.id}`}>{entry.maryText}</p>
        </div>
      </div>
    </motion.div>
  );
  ...
  return (
    <div className="min-h-[100dvh] w-full bg-background flex flex-col pb-24 items-center">
      <div className="w-full max-w-[430px] flex-1 flex flex-col px-6 pt-8">
        ...
        <h1 className="text-2xl font-bold text-center text-foreground mb-6" data-testid="text-page-title">
          Review Log
        </h1>

        <div className="flex items-center gap-4 mb-7">
          <MaryAvatar height={120} showEmote={false} outfit={gs.equippedOutfit} emote={emote} className="scale-90 origin-left" />
          <motion.div
          ...
          >
            <p className="text-sm font-medium text-foreground">Here are our conversations.</p>
          </motion.div>
        </div>

        {/* Level Tabs */}
        <div className="flex gap-2 mb-6 overflow-x-auto pb-1" data-testid="level-tabs">
          <button
            onClick={() => setActiveTab("current")}
            className={`shrink-0 px-4 py-2 rounded-full text-sm font-bold transition-all ${activeTab === "current" ? "bg-primary text-primary-foreground shadow-sm" : "bg-card border border-border text-muted-foreground hover:border-primary/50"}`}
          ...
          {Array.from({ length: currentLevel - 1 }, (_, i) => currentLevel - 1 - i).map((lvl) => (
            <button
              ...
              className={`shrink-0 px-4 py-2 rounded-full text-sm font-bold transition-all ${activeTab === lvl ? "bg-primary text-primary-foreground shadow-sm" : "bg-card border border-border text-muted-foreground hover:border-primary/50"}`}
        ...
        <div className="flex items-center gap-2 mb-4">
          ...
          <span className="text-xs font-bold text-muted-foreground px-2">
            Level {resolvedLevel}{activeTab === "current" ? " (current)" : ""}
          </span>
          <div className="h-px flex-1 bg-border" />
        </div>
          ...
          {filtered.length === 0 ? (
            ...
            >
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground/70 mt-1">Complete a talk session to see it here.</p>
              </div>
            </motion.div>
          ) : (
            <motion.div
